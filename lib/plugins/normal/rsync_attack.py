...
def check(ip, portinfo, timeout):
    if portinfo['port']==873 or 'rsync' in portinfo['type']:
        info = ''
        not_unauth_list = []
        weak_auth_list = []
        try:
            rwc = RsyncWeakCheck(ip, portinfo['port'])
            for path_name in rwc.get_all_pathname():
                ret = rwc.is_path_not_auth(path_name)
                if ret == 0:
                    not_unauth_list.append(path_name)
                elif ret == 1:
                    for user_pass in open("dict/rsync.dic", "r"):
                        tmp = user_pass.strip()
                        _ = tmp.split()
                        user = _[0]
                        passwd = _[1]
                        try:
                            res = rwc.weak_passwd_check(path_name, user, passwd)
                            if res:
                                weak_auth_list.append((path_name, user, passwd))
        ...
        if info:
            plugin_info['vulinfo'] = 'rsync unauthed '
            return 1
        return 2
    return 0
